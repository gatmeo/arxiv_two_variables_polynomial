\section{The type \mathintitle{A} Springer resolution}
\label{chapter_springer}

We recall the parts of Springer theory used in the proof, following
\cite{chriss1997representation}.

\subsection{Notation}

Let $G$ be a connected complex semisimple group with Lie algebra
$\lie{g}$. Fix a Borel subgroup $B\subseteq G$, with maximal torus
$T\subseteq B$ and unipotent radical $U$. Write $\lie{b}$, $\lie{h}$,
and $\lie{n}$ for their Lie algebras, $\Phi=\Phi^+\sqcup\Phi^-$ for the
corresponding roots, and $W=N_G(T)/T$ for the Weyl group. The Killing
form identifies $\lie{g}$ with $\lie{g}^*$. Let $\borel=G/B$ be the
flag variety, let $\nilcone\subseteq\lie{g}$ be the nilpotent cone, and
write $\lie{g}^{\mathrm{sr}}$ and $\lie{h}^{\mathrm{reg}}$ for the
regular semisimple and regular loci, respectively.

We use the following standard consequences of the Bruhat decomposition:
\begin{lemma}[lem:bruhat_decomposition]{Bruhat decomposition}
  There are natural bijections
  \[
    W\cong\borel^T\cong B\backslash G/B
    \cong G\backslash(\borel\times\borel).
  \]
  The $B$-orbits $\schubert_w=BwB/B$ form the Schubert-cell
  decomposition of $\borel$, and
  \[
    \schubert_w\cong\lie{n}\cap w\cdot\lie{n}^-.
  \]
\end{lemma}
...
The standard specialization construction gives the following result;
see \cite[Section 3]{chriss1997representation}.
\begin{theorem}[thm:steinberg_group_algebra]{}
  There is an algebra isomorphism
  \[
    \HBMtop(\steinberg)\cong\mathbb{C}[W].
  \]
  Under this isomorphism, the anti-involution induced by interchanging
  the two factors of $\steinberg$ corresponds to $w\mapsto w^{-1}$.
\end{theorem}
...
The convolution construction makes $\HBM_\bullet(\borel_x)$ a
$\mathbb{C}[W]$-bimodule for $x\in\nilcone$. The two actions are
related by $w\mapsto w^{-1}$, so the left representation is dual to
the right representation. Since $\borel_x$ is compact,
$\HBM_\bullet(\borel_x)^*\cong\HH^\bullet(\borel_x)$.
...
For $x = 0$, we have $\borel_0 = \borel$. The $\HBMtop(\steinberg)$-action on $\HH^\bullet(\borel)$ and on $\HH^\bullet(\tilde{\nilcone})$ are identified. Under the homotopy equivalences $G/T\to\borel\gets\tilde{\nilcone}$, this is the usual Weyl-group action on the cohomology of $G/T$.
...
\begin{proposition}[prop:convolution_action_on_cohomology]{}
...
\subsection{Springer Theory in Type \mathintitle{A}}
